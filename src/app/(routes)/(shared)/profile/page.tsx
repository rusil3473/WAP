...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const data = response.data?.data as AuthProfile | undefined;
      if (!data) {
  ...
  const saveProfile = async () => {
    ...
    try {
      const response = await axios.put("/api/auth/profile", { fullName });
      const updated = response.data?.data as AuthProfile | undefined;
      if (!updated) {
      ...
      <main className="container mx-auto flex-grow px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl rounded-2xl bg-white p-6 shadow-sm sm:p-8">
          ...
          <button
            ...
            className="mt-8 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-blue-400"
          >
